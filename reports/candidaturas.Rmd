...
```{r}
racas_todas = prefs %>%
  group_by(sigla_uf, nome_municipio, codigo_ibge, descricao_cor_raca) %>%
  summarise(
    n = n()
  ) %>% 
...
racas_wide = racas_todas %>% pivot_wider(
  id_cols = c(codigo_ibge, candidaturas, sigla_uf), 
...
racas_todas %>% 
  write_csv("candidaturas-2020-racas-long.csv")

racas_wide %>% 
  write_csv("candidaturas-2020-racas-todas.csv")

racas_prevalentes %>% 
  write_csv("candidaturas-2020-raca-maiscomum.csv")
```
...
```{r}
generos_long = prefs %>%
  group_by(sigla_uf, nome_municipio, codigo_ibge, descricao_sexo, capital) %>%
  summarise(
    n = n()
  ) %>% 
  mutate(candidaturas = sum(n)) %>% 
...
generos_wide = generos_long %>% 
  pivot_wider(
  id_cols = c(codigo_ibge, candidaturas, nome_municipio, sigla_uf, capital), 
...
generos_long %>% 
  write_csv("candidaturas-2020-genero_long.csv")

generos_wide %>% 
  write_csv("candidaturas-2020-genero_wide.csv")
